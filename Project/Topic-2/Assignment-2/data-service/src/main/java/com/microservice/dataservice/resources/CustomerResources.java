...
public class CustomerResources {
	...
	public CustomerEntity getCustomerById(@PathVariable Long id) throws NoSuchCustomerFoundException{
		...
		return customerRepository.findById(id).orElseThrow(()-> new NoSuchCustomerFoundException("No Customer with Id: "+id));
	}
	...
	@PutMapping(value = "/{id}")
	public CustomerEntity updateCustomer(@PathVariable Long id, @Valid @RequestBody CustomerEntity customer ) throws NoSuchCustomerFoundException {
		return customerRepository.findById(id)
				.map((customerEntity)->{
					...
					//TODO: verify the type of customer
	...
	/*
	 * @GetMapping(value = "/{id}/payments") public List<PaymentEntity>
	 * getPaymentsByCustomerId(@PathVariable Long id){
	 * 
	 * List<PaymentEntity> paymentEntity = paymentRepository.findByCustomerId(id);
	 * return paymentEntity; }
	 * 
	 * @GetMapping(value = "/{customerid}/payments/{paymentId}") public
	 * PaymentEntity getPaymentsByCustomerId(@PathVariable Long
	 * customerid, @PathVariable Long paymentId) throws NoSuchPaymentFoundException,
	 * NoSuchCustomerFoundException { if(!customerRepository.existsById(customerid))
	 * throw new NoSuchCustomerFoundException("No Customer with Id: "+customerid);
	 * PaymentEntity ent= paymentRepository.findByPaymentIdAndCustomerId(paymentId,
	 * customerid) .orElseThrow(()-> new
	 * NoSuchPaymentFoundException("No Payment id:"+paymentId
	 * +" for customer id:"+customerid)); return ent; }
	 */	
}
